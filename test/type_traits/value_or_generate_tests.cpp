...
{
    auto generate_default = [] { return default_arg(); };
    decltype(auto) value(meta::value_or_generate(input_value, generate_default));
    decltype(auto) option(meta::value_or_generate(input_option, generate_default));
...
TEST(value_or_generate_tests, test_value_or__default_default__ok)
{
...
TEST(value_or_generate_tests, test_value_or__default_int__ok)
{
    default_arg::destructor_counter() = 0;
    int option = 100;
    ASSERT_EQ(tfunc(meta::default_v, option), "D1000 V101");
    ASSERT_EQ(option, 101);
    ASSERT_EQ(default_arg::destructor_counter(), 1);
}

TEST(value_or_generate_tests, test_value_or__int_int__ok)
{
    default_arg::destructor_counter() = 0;
    int option = 100;
    ASSERT_EQ(tfunc(200, option), "V200 V101");
    ASSERT_EQ(option, 101);
    ASSERT_EQ(default_arg::destructor_counter(), 0);
}
